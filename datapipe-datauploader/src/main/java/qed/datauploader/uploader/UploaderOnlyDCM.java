...
public class UploaderOnlyDCM implements Runnable{
    ...
    public UploaderOnlyDCM(){}
    public UploaderOnlyDCM(List<String> list){
        this.list = list;
    ...
    private void uploadBatch(List<String> dicomDirList){
        ...
        for(String dir : dicomDirList) {
            ...
            if (numberOfSlices == -1) {  //文件丢失或顺坏
                ...
                if(-1 == verifySpecialDicom(seriesDir)){
                    logger.log(Level.ERROR,"该序列存在错误:"  + dir);
                    continue;
                }
                uploadSpecial(dir);
                continue;
...
//            System.out.println(metaMsg.toJSONString());
    ...
    private void doFormatMeta(JSONObject metaMsg) {
        ...
        int patientAge = DataUploaderTool.getPatientAge(metaMsg.getString(SysConsts.SeriesDate_TAG), metaMsg.getString(SysConsts.PatientBirthDate_TAG));
        if(patientAge==0){
            String patientAgeStr = metaMsg.getString(SysConsts.PatientAge_TAG);
            patientAgeStr = patientAgeStr.substring(0,patientAgeStr.indexOf("Y"));
            if(!StringUtils.isBlank(patientAgeStr))
                patientAge = Integer.parseInt(patientAgeStr);
            metaMsg.put(SysConsts.PatientAge_TAG,patientAge);
        }else{
            metaMsg.put(SysConsts.PatientAge_TAG,patientAge);
        }
...
//                        System.out.println(metaMsg.getString(SysConsts.SeriesUID));
                ...
                if(success){
                    ...
                    System.out.println(success);
                    DataUploaderTool.recordLog(logger,file.getAbsolutePath(), true, "hbase,hdfs上传失败");
                }
            }
        }
        return true;
    ...
    private int verifySpecialDicom(File seriesDir){
        //经过初步检验每个dcm中ImageNumber图片序列都为1.但是有可能是错误图片，即：ImageNumber都为1，但是SeriesId都是一样的。
        Set<String> set = new HashSet<String>();
        int sum = -1;
        List<Integer> list = new ArrayList<Integer>();
        for(File e : seriesDir.listFiles()){
            JSONObject instanceNumberField = IMAParseTool.extractExactField(e.getAbsolutePath(),SysConsts.InstanceNumber_TAG);
            JSONObject seriesInstanceUIDField = IMAParseTool.extractExactField(e.getAbsolutePath(),SysConsts.SeriesInstanceUID_TAG);
            if(!instanceNumberField.isEmpty()){
                int imageNumber = Integer.parseInt(instanceNumberField.getString(SysConsts.InstanceNumber_TAG));
                set.add(seriesInstanceUIDField.getString(SysConsts.SeriesInstanceUID_TAG));
                list.add(imageNumber);
            }
        }
        if(list.size()!=0){
            ...
            if(code==1){//连续递增,常规处理
                ...
                if(list.size() !=1 && set.size()==1){
                    return -1;  //这中情况是因为一个序列下有多个dcm, 每个dcm中只有一个图片，但是他们InstanceNumber都是1,这是错误的。
                }
                return 0;  //正常格外处理
            }
        }
