...
public class KFBUploader {
    ...
    public int upload(String dir) {
        ...
        String suffix = "kfb";
        List<String> kfbPaths = DataUploaderTool.listEdfPath(dir,suffix);
        if(kfbPaths != null && kfbPaths.size() != 0){
            success = true;
        }

        //循环上传每一个kfb文件
        if(success){
            for(String e : kfbPaths){
                uploadSingle(e);
            }
        }
...
//        String PatientName = tempname.substring(3,tempname.length());
        ...
        String CreateDate = DataUploaderTool.getLastModifyDateOfFile(kfbPath);   //采用edf文件的最后修改时间
        String InstitutionName = uploaderconf.getKfbHospital(); //医院，配置文件获取
        String entrydate = DataUploaderTool.getTodayDate();
        //配置文件获取目录前缀，CreateDate转化为/年/月/日/MD5(patientuid)_脑电原文件名.edf
        String hdfspath = uploaderconf.getDirPrefixKfb()
        ...
        if(success){
            try {
                if(SysConsts.FAILED == hbaseService.putOne(uploaderconf.getKfbTablename()
                        ,uploaderconf.getKfbCf(),meta)){
                    success = false;
                    if(false == hdfsService.del(meta.getString(SysConsts.HDFSPATH),hdfsconf)){
                        logger.log(org.apache.log4j.Level.ERROR,"删除hdfs上kfb文件失败："+meta.getString(SysConsts.HDFSPATH));
                    }
                }
        ...
        if(success){
            if(SysConsts.FAILED == elasticSearchService.insertOne(uploaderconf.getIndexKfb(),
                    uploaderconf.getTypeKfb(),null,meta)){
                success = false;
            }
    ...
    public static void main(String[] args) {
        String str = "F:\\实验室\\kfb";
        new KFBUploader().upload(str);
//        FileSystem fs;
//        try {
