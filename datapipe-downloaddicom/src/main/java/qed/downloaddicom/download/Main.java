...
import qed.downloaddicom.conf.SysConstants;
...
//        doDownload(confFilePath,desDir);
        ...
        if(!StringUtils.isBlank(confFilePath) && !StringUtils.isBlank(desDir) && checkParameter(args))
            doDownload(confFilePath,desDir);
        else
            printUsage();
    ...
    public static boolean checkParameter(String[] args){
        ...
        if(!(new File(desDir).exists())){
            ...
            logger.error("指定文件存放目录不存在,已创建");
        }
        return true;
    }

    public static void printUsage(){
        System.out.println("Usage :   java -jar toground.jar conf des\n\tconf:json下载文件\n\tdestination:下载的dicom文件存放目录，需要提前创建" );
    }
}
